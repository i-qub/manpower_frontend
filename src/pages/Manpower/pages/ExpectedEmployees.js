...
function ExpectedEmployees(props) {
    ...
    useEffect(() => {
        const { subdept } = props;
        const { shift } = props;
        axios.post(newURL+'/mp/getManpowerData', { subdept: subdept, shift: shift, verified: 'NO' }).then((response) => {
        ...
        <div>
            <MuiThemeProvider theme={theme}>
                <MaterialTable
                    ...
                    components={{
                        Action: props => {
                            const [verifyID, setVerified] = useState(props.data.verified);
                            if (verifyID === 'NO') {
                                return (
                                    <Button
                                        onClick={(event) => { props.action.onClick(event, props.data); setVerified('YES') }}
                                        color="primary"
                                        variant="contained"
...
export default ExpectedEmployees;